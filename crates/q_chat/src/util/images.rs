...
///
/// This is not used currently, but can be used in the future for handling user prompts directly.
...
/// For now, specifically used to handle screenshots from MacOS.
/// An example of a screenshot name is: /path-to/Screenshot 2025-03-13 at 1.46.32 PM.png
/// If it looks like a Mac screenshot, it replaces the whitespace between seconds and AM/PM with an
/// NNBSP character.
...
pub fn handle_images_from_paths(output: &mut impl Write, paths: &[String]) -> RichImageBlocks {
    let user_prompt = paths
        .iter()
        .map(|path| format!("'{}'", pre_process(path)))
        .collect::<Vec<_>>()
        .join(" ");
